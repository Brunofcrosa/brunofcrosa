<!-- Header Banner -->
<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:1a1a2e,100:0d2137&height=130&section=header&text=Bruno%20Fetzer&fontSize=42&fontColor=ffffff&animation=fadeIn&fontAlignY=55&desc=Full%20Stack%20Developer%20%7C%20UFSM&descAlignY=75&descSize=18" width="100%"/>
</div>

<!-- Social Links -->
<div align="center">
  <a href="https://www.linkedin.com/in/brunofcrosa" target="_blank" rel="noopener noreferrer">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" height="28" alt="LinkedIn" />
  </a>
  &nbsp;
  <a href="http://instagram.com/brunofcrosa" target="_blank" rel="noopener noreferrer">
    <img src="https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white" height="28" alt="Instagram" />
  </a>
  &nbsp;
  <a href="mailto:brunofcrosa@hotmail.com" target="_blank" rel="noopener noreferrer">
    <img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" height="28" alt="Email" />
  </a>
  &nbsp;
  <a href="https://curriculo-azure.vercel.app/" target="_blank" rel="noopener noreferrer">
    <img src="https://img.shields.io/badge/Currículo-0078D4?style=for-the-badge&logo=microsoftword&logoColor=white" height="28" alt="Currículo" />
  </a>
  &nbsp;
  <a href="https://portfolio-brunofcrosas-projects.vercel.app/" target="_blank" rel="noopener noreferrer">
    <img src="https://img.shields.io/badge/Portfólio-28A745?style=for-the-badge&logo=vercel&logoColor=white" height="28" alt="Portfólio" />
  </a>
</div>

<br/>

<!-- Typing Animation -->
<div align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=22&duration=3000&pause=1000&color=58A6FF&center=true&vCenter=true&width=620&lines=Full+Stack+Developer;Java+17+%7C+Spring+Boot+3+%7C+Angular+17%2B;OAuth2+%26+Keycloak+%7C+Clean+Architecture;Microsservi%C3%A7os+%7C+Flutter+%26+Dart" alt="Typing SVG" />
</div>

---

## 👨‍💻 Sobre mim

```java
public class Bruno {
    String cargo      = "Full Stack Developer @ LUMAC RS – Colégio Politécnico UFSM";
    String graduacao  = "Tecnólogo em Sistemas para Internet – UFSM (previsão 2027)";
    String tecnico    = "Informática para Internet – Colégio Politécnico UFSM (2025)";
    String certificado = "Full Stack – SENAI (2024)";
    String stack      = "Java 17 · Spring Boot 3 · Angular 17+";
    String seguranca  = "Keycloak · OAuth2 · OpenID Connect";
    String arquitetura = "Clean Architecture · Microsserviços · REST APIs";
    String aprendendo = "Flutter / Dart";
}
```

- 🏢 **Desenvolvedor Full Stack** na LUMAC RS, atuando com **Java 17**, **Spring Boot 3** e **Angular 17+**  
- 🔐 Implementação de segurança com **Keycloak** (OAuth2 / OpenID Connect)  
- ⚙️ Foco em **Clean Architecture**, APIs escaláveis e microsserviços  
- 🎓 Técnico em Informática pelo **Colégio Politécnico da UFSM** (2025) e Full Stack pelo **SENAI** (2024)  
- 📱 Explorando desenvolvimento mobile com **Flutter & Dart**  

---

## 🛠️ Stack Tecnológica

### ⚙️ Back-end
<div align="center">
  <img src="https://skillicons.dev/icons?i=java,spring,nodejs,cs,python&theme=dark" />
</div>

### 🖥️ Front-end
<div align="center">
  <img src="https://skillicons.dev/icons?i=angular,typescript,react,javascript,html,css,bootstrap,tailwind&theme=dark" />
</div>

### 📱 Mobile
<div align="center">
  <img src="https://skillicons.dev/icons?i=flutter,dart&theme=dark" />
</div>

### 🗄️ Banco de Dados
<div align="center">
  <img src="https://skillicons.dev/icons?i=postgres,mysql&theme=dark" />
</div>

### 🛠️ DevOps & Ferramentas
<div align="center">
  <img src="https://skillicons.dev/icons?i=docker,git,github,idea,webstorm,vscode,figma,postman&theme=dark" />
</div>

---

## 📌 Projetos em Destaque

<div align="center">
  <a href="https://github.com/Brunofcrosa/portfolio">
    <img src="https://github-readme-stats.vercel.app/api/pin/?username=Brunofcrosa&repo=portfolio&theme=github_dark&hide_border=true&cache_seconds=1800" />
  </a>
  <a href="https://github.com/Brunofcrosa/imobiliaria">
    <img src="https://github-readme-stats.vercel.app/api/pin/?username=Brunofcrosa&repo=imobiliaria&theme=github_dark&hide_border=true&cache_seconds=1800" />
  </a>
  <a href="https://github.com/Brunofcrosa/comanda_app">
    <img src="https://github-readme-stats.vercel.app/api/pin/?username=Brunofcrosa&repo=comanda_app&theme=github_dark&hide_border=true&cache_seconds=1800" />
  </a>
  <a href="https://github.com/Brunofcrosa/estoque">
    <img src="https://github-readme-stats.vercel.app/api/pin/?username=Brunofcrosa&repo=estoque&theme=github_dark&hide_border=true&cache_seconds=1800" />
  </a>
</div>

---

## 📊 GitHub Stats

<div align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=Brunofcrosa&show_icons=true&theme=github_dark&hide_border=true&count_private=true&include_all_commits=true&cache_seconds=1800" height="165" alt="GitHub Stats" />
  &nbsp;
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Brunofcrosa&layout=compact&theme=github_dark&hide_border=true&langs_count=8&cache_seconds=1800" height="165" alt="Top Languages" />
</div>

<br/>

<div align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com?user=Brunofcrosa&theme=github-dark-blue&hide_border=true&locale=pt_BR" alt="GitHub Streak" />
</div>

---

<!-- Footer Banner -->
<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:0d2137,100:1a1a2e&height=80&section=footer" width="100%"/>
</div>
